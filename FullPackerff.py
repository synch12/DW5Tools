...
def pack_file(path,suffix):
    ...
    out_stream = open(path[:-1]+'.dat', 'wb')


    for p in paths:
        onlyfiles = [f for f in listdir(p) if isfile(join(p+'\\', f))]
        [print("isfile check : " +str(f) +" "+ str(isfile(join(p+'\\', f)))) for f in listdir(p) if isfile(join(p+'\\', f))]
        print(onlyfiles)
        basefiles = [i.split('_', 1)[0] for i in onlyfiles]
        files = [*set(basefiles)]
        files.sort(key = int)
        namedfiles = [glob.glob(p+i+suffix+'*')[-1] for i in files if isfile(glob.glob(p+i+suffix+'*')[0])]
        ...
        for i in namedfiles:
            ...
            in_stream.close()
        print(hex(0xC400-(out_stream.tell()%0xC400)))
        length = (0xC400-(out_stream.tell()%0xC400))if not(out_stream.tell()%0xC400) <= 0x10 else 0
        if(length):
            out_stream.write(bytearray(length))
    out_stream.close()
